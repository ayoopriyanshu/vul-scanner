...
def helper():
        ...
        print("\tCtrl+Z: Quits VUL SCANNER.")
...
A GO-TO LINUX TOOL THAT EXPOSES THE LOOPHOLES OF YOUR SITE
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"VUL SCANNER received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
if args_namespace.help or (not args_namespace.update \
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+" VUL SCANNER was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by VUL SCANNER."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. VUL SCANNER will still perform the rest of the tests. Install these tools to fully utilize the functionality of VUL SCANNER."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit VUL SCANNER.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
